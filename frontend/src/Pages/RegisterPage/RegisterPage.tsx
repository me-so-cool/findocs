...
const RegisterPage = (props: Props) => {
  ...
  const handleRegister = async (form: RegisterFormsInputs) => {
    ...
    console.log("API response (errors):", result);

    // Check if the result is an array of errors
    if (Array.isArray(result) && result.length > 0) {
      setApiErrors(result.map((err: any) => err.description));  // Collect all error descriptions
    } else {
      setApiErrors([]);  // Clear errors on successful registration
    }
  };
